...
def add_event_user():
    ...
    if (etat == 0):
        ...
        return jsonify({"message": "Impossible d'ajouter l'event à l'utilisateur"})

'''
Supprime un utilisateur d'un groupe
Méthode POST : Argument : user_id et group_id
Retourne 0 si succès, 2 si l'utilisateur n'est pas dans le groupe, et 1 si un des deux n'existe pas 
'''
@app.route('/quitter-groupe', methods=['POST'])
def quitter_groupe():
    group_id = request.json.get('group_id')
    user_id = request.json.get('user_id')

    etat = supprime_utilisateur_et_groupe(user_id, group_id)

    if (etat == 0):
        return jsonify({"message": "Groupe supprimé"}), 201
    elif (etat == 2):
        return jsonify({"message": "Utilisateur n'appartient pas au groupe"})
    else :
        return jsonify({"message": "Groupe ou user inexistant"})
