...
def ajouter_evenement_user(event_json, user_id):
    ...
    if user_doc.exists:
        ...
        print("Événement ajouté avec succès a l'user", user_id)
        return 0
    else:
...
'''
Supprime un utilisateur d'un groupe et le groupe de la liste des groupes de l'utilisateur
Prend en argument l'identifiant de l'utilisateur et l'identifiant du groupe
Renvoie 0 si succès, 1 si l'utilisateur ou le groupe n'existe pas dans la base de données, 2 si l'utilisateur n'appartient pas au groupe
'''
def supprime_utilisateur_et_groupe(user_id, group_id):
    # Référence de l'utilisateur dans la base de données
    user_ref = db.collection("users").document(user_id)
    # Vérifie si l'utilisateur existe
    if not user_existe(user_id):
        print(f"L'utilisateur '{user_id}' n'existe pas dans la base de données.")
        return 1
    ...
    if not group_doc.exists:
        print(f"Le groupe '{group_id}' n'existe pas dans la base de données.")
        return 1

    # Obtenir les données du groupe
    group_data = group_doc.to_dict()
    # Obtenir la liste des utilisateurs du groupe
    users = group_data.get("users", [])

    # Vérifie si l'utilisateur appartient au groupe
    if user_id not in users:
        print(f"L'utilisateur '{user_id}' n'appartient pas au groupe '{group_id}'.")
        return 2

    # Supprime l'utilisateur de la liste des utilisateurs du groupe
    users.remove(user_id)
    group_ref.update({"users": users})
    print(f"L'utilisateur '{user_id}' a été supprimé du groupe '{group_id}' avec succès.")
